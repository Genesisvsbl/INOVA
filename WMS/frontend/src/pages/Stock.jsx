...
export default function Stock() {
  ...
  const imprimirBloqueados = () => {
    const rows = (bloqueados || []).map((row) => `
      <tr><td>${escapeHtml(row.codigo_material)}</td><td>${escapeHtml(row.descripcion_material)}</td><td>${escapeHtml(row.ubicacion)}</td><td>${escapeHtml(row.lote_almacen || "-")}</td><td>${escapeHtml(row.lote_proveedor || "-")}</td><td>${escapeHtml(row.fecha_vencimiento || "-")}</td><td class="num">${escapeHtml(formatQty(row.cantidad_r ?? row.cantidad ?? 0))}</td></tr>`).join("");
      ...
      {showBloqueados && (
        <div style={{ border: `1px solid ${colors.badBd}`, borderRadius: 12, background: "#fff", overflow: "hidden" }}>
          <div style={{ padding: 14, borderBottom: `1px solid ${colors.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}><button type="button" onClick={imprimirBloqueados} style={{ height: 36, borderRadius: 10, border: `1px solid ${colors.border}`, background: colors.blue, color: "#fff", fontWeight: 900, padding: "0 12px", cursor: "pointer" }}>Imprimir reporte</button><button type="button" onClick={() => setShowBloqueados(false)} style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${colors.border}`, background: "#fff", cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", color: colors.text }} aria-label="Cerrar reporte de bloqueados"><X size={16} /></button></div>
          </div>
          <div style={{ overflowX: "auto" }}>
