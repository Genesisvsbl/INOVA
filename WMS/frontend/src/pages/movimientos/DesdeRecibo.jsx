...
// Nivel = dígito justo ANTES del apóstrofe. Ej: 300111'02 -> nivel 1.
function nivelDeUbicacion(u) {
  const code = codeUbic(u);
  const ap = code.indexOf("'");
  return ap > 0 ? code[ap - 1] : "";
}
// Módulo = dígito DOS posiciones antes del apóstrofe. Ej: 300111'02 -> módulo 1.
...
export default function DesdeRecibo() {
  ...
  // Ubicaciones de la base/zona elegida -> opciones de módulo y nivel.
  ...
  const imprimirVacia = () => {
    if (!tbLinea || !tbSel) return;
    const code = normalizarUbicacionPantalla(tbSel.ubicacion || `${tbSel.ubicacion_base || ""}${tbSel.posicion || ""}`);
  ...
  const confirmarVacia = () => {
    if (!tbLinea || !tbSel) {
      showWmsAlert("Selecciona una ubicación de la lista.");
      return;
    }
    const idx = tbLinea.idx;
    const base = String(tbSel.ubicacion_base || "").trim();
    const posicion = String(tbSel.posicion || "").trim();
    const ubicacion = normalizarUbicacionPantalla(tbSel.ubicacion || `${base}${posicion}`);
    setUbicPorLinea((prev) => {
      const actual = prev[idx] || {};
      return {
        ...prev,
                ...
                <div style={{ display: "flex", alignItems: "flex-end" }}>
                  <button
                    ...
                    style={{
                      ...
                      color: "#fff",
                      fontWeight: 800,
                      cursor: tbLoading ? "default" : "pointer",
                      display: "inline-flex",
                      alignItems: "center",
                      gap: 6,
                  ...
                  {tbView.length === 0 ? (
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap", justifyContent: "flex-end" }}>
                        ...
                        >
                          <Check size={16} /> Confirmar en la línea
                        </button>
                      </div>
                    </>
                  )}
